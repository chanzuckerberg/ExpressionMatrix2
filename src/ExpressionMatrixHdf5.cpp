	...
	try {
		...
		for(size_t i=0; i<hdf5CellNames.size(); i++) {
			...
			try {
				...
				cout << "Error occurred adding cell " << i << " " << hdf5CellNames[i] << " from HDF5 file " << fileName << endl;
				cout << "Index pointers information: " << indexPointers[i] << " " << indexPointers[i+1] << " " << n << endl;
