...
if not directoryExists('src') or not directoryExists('doc') or not directoryExists('Release'):
...
# Strip the shared library, then add it to the arechive. 
# Stripping it reduces its size by a large factor.    
os.system('tar --create --file Release.tar doc')
os.chdir('Release')  
